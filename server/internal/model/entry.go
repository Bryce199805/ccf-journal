...
type Entry struct {
	...
	CCFRelations      *string  `json:"ccf_relations"`
	...
	WoSStatus         *string  `json:"wos_status"`
	WoSReason         *string  `json:"wos_reason"`
	JIF               *string  `json:"jif"`
...
func ScanEntry(row *sql.Row) (*Entry, error) {
	...
	err := row.Scan(
		...
		&e.CCFPublisher, &e.CCFUrl, &e.CCFRelations, &e.LetPubUrl, &e.JournalID, &e.Name, &e.JournalAbbr,
		...
		&e.Xinrui, &e.CAS2025, &e.CAS2023, &e.WoSZone, &e.WoSStatus, &e.WoSReason,
		&e.JIF, &e.JCIJson, &e.CitescoreRankings,
		&e.IsCCF, &e.CatalogSource, &e.InclusionReason, &e.LastScrapedAt, &e.LastScrapeError,
	)
	if err != nil {
		return nil, err
	}
...
type ListQuery struct {
	...
	Top       bool   `form:"top"`
	Tag       string `form:"tag"`
	Catalog   string `form:"catalog" binding:"omitempty,oneof=ccf non_ccf"`
...
// WoSZones returns the list of WOS zones (comma-separated)
func (q *ListQuery) WoSZones() []string {
	return splitMulti(q.WoSZone)
}
...
type Stats struct {
	...
	CCFJournals      int            `json:"ccf_journals"`
	NonCCFJournals   int            `json:"non_ccf_journals"`
